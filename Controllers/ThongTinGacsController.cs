...
namespace MyPhamCheilinus.Controllers
{
    ...
    public class ThongTinGacsController : Microsoft.AspNetCore.Mvc.Controller
    {
        ...
        {
            var donVis = await _context.DonVis.Where(d => d.Gac == true).ToListAsync();

            return View(donVis); // View phải nhận Model là IEnumerable<DonVi>
        }
        ...
        public IActionResult CheckEvents_D1(string date)
        {
            try
            {
                using (var _contextContext = new CanhGacContext())
                {
                    ...
                    bool hasEvents = _contextContext.ThongTinGacs.Any(e => e.Ngay == selectedDate && (e.MaDonVi == "C159" || e.MaDonVi == "C156" || e.MaDonVi == "C157" || e.MaDonVi == "C154"));
