        ...
        public async Task<IActionResult> Login(LoginViewModel customer, string? returnUrl = null)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (canBo == null)
                    {
                        _notifyService.Success("Tài khoản không tồn tại. Vui lòng đăng ký tài khoản.");
                        return RedirectToAction("DangKyTaiKhoan");
                    }

                    if (string.IsNullOrEmpty(customer.Password))
                    {
                        _notifyService.Success("Password không được để trống.");
                        return View(customer);
                    }

                    string salt = canBo.Salt?.Trim() ?? string.Empty;  // Xử lý null cho canBo.Salt
                    string pass = (customer.Password + salt).ToMD5();
                    ...
                    var claims = new List<Claim>
            {
                ...
                new Claim("AccountId", canBo.AccountId.ToString()),
                    ...
                    else if (canBo.RoleId == GetRoleIdForCustomer())
                    {
